...
def compr_sites_distribution(infile,outdir,outname,title): 
    ...
    plt.title(title)
    plt.legend(borderaxespad=0.1,labelspacing=.1,
    ...
    df_out = pd.DataFrame()
    for flag_distance in np.arange(20,1000,20):
    ...
    df_out.to_csv('{}/{}.csv'.format(outdir,outname))
...
# == check the ctcf union peaks
infile='f1_bedtools_closest/ctcf_union_peak_closest.tsv'
outname = 'ctcf_union_peak'
title = 'CTCF peak'
compr_sites_distribution(infile,outdir,outname,title)


# == check the ctcf motifs
infile='f1_bedtools_closest/ctcf_motif_closest.tsv'
outname = 'ctcf_motif'
title = 'CTCF motif'
compr_sites_distribution(infile,outdir,outname,title)


# == check the notch motifs
infile='f1_bedtools_closest/notch_motif_closest.tsv'
outname = 'notch_motif'
title = 'NOTCH'
compr_sites_distribution(infile,outdir,outname,title)
