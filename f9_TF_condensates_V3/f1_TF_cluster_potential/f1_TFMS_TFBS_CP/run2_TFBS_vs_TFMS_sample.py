...
def main(dcID):
    ...
    tfbs_types = ['TFBS','TFBS_overlap_motif','TFBS_NOT_overlap_motif']
    ...
    for tfbs_type in tfbs_types:
        ...
        # ==== cistrome info
        df_qc = pd.read_csv('../../../f8_TF_condensates_V2/data/cistrome2019_selected_QC.csv',index_col=0)
        factor = df_qc.loc[dcID].Factor 
        celltype = df_qc.loc[dcID].Cell_line 
        # ==== peak file
        peak_dir = '../f0_bedtools_closest/data_{}'.format(tfbs_type)
        if tfbs_type == 'TFBS':
            peak_file = '{}/{}_sort_peaks.narrowPeak.tsv'.format(peak_dir,dcID)
        else:
            peak_file = '{}/{}_{}_{}.tsv'.format(peak_dir,dcID,factor,celltype)
        # ==== SE data    
        ...
        # ==== outcome info
        outname = '{}_{}_{}'.format(dcID,factor,celltype)
