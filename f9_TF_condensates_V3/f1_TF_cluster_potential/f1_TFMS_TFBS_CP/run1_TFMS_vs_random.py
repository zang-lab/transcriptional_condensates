...
def compr_sites_distribution(df_cp,infile,outdir): 
    ...
    df_cp.loc[outname,'log10-dis T-test-s'] = s.round(2)
    df_cp.loc[outname,'log10-dis T-test-p'] = '{:.2e}'.format(p) 
    s,p = scipy.stats.ranksums(sample_vals_log,values_log,)
    df_cp.loc[outname,'log10-dis Wilcoxon-rank-sum-s'] = s.round(2)
    df_cp.loc[outname,'log10-dis Wilcoxon-rank-sum-p'] = '{:.2e}'.format(p) 
    return df_cp
...
df_cp.index.name = 'TF'
df_cp = df_cp.sort_values(by='log10-dis T-test-s',ascending=False)
