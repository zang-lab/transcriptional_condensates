...
indirs = ['CP_TFBS_vs_TFMS','CP_TFBS_overlap_motif_vs_TFMS','CP_TFBS_NOT_overlap_motif_vs_TFMS']
...
or_thre = {'CP_Ttest':8,
           'CP_RankSum':8,
           'SE':5}  
label_name = {'CP_Ttest':'log2 CP',
              'CP_RankSum':'log2 CP',
              'SE':'log2 OR'}
cmp_dict   = {'CP_Ttest':plt.cm.RdPu,
              'CP_RankSum':plt.cm.RdPu,
              'SE':plt.cm.bwr}
...
for indir in indirs[:]:
    ...
    for or_key in list(group_info.keys())[:]:
        for fisher_P_key in list(group_info[or_key].keys())[1:]:
            ...
            df_s_z.round(4).to_csv(outdir+os.sep+'_csv/{}_statistics_zscored.csv'.format(basename))
            ...
            pal = cmp_dict[or_key]
            vmax = or_thre[or_key]
            vmin = -1*vmax if or_key == 'SE' else 0 
            ...
            cbar.set_label(label_name[or_key],labelpad=-50 if or_key == 'SE' else -45,
