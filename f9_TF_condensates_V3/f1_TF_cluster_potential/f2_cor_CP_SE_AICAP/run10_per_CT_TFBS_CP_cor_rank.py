...
def scatter_plot(x,y,xlabel,ylabel,figname,title):
    ...
    for label_index in y.index[:3]:
        plt.scatter(label_i,y[label_index],
                    c=colors1[label_i],
    ...
    plt.legend(bbox_to_anchor=[.45 if title=='IMR90' else .51,1.01],
    ...
    # plt.text(.03,.9,'$R$ = {:.2f}'.format(r_value),fontsize=11,
...
outdir = 'f10_per_CT_TFBS_CP_cor_rank'
...
for tfbs_cp_type in tfbs_cp_types[:1]:
    ...
    for cp_type in cp_types[1:]:
        ...
        for ct in tfbs_cp_s.columns[:]:
            ...
            # outdf = pd.concat([x,y],axis=1)
            # outdf['cp rank'] = y.rank(ascending=False)
            # outdf['z-score rank'] = x.rank(ascending=False)
            # outdf['avg rank'] = .5*(y.rank(ascending=False)+y.rank(ascending=False))
