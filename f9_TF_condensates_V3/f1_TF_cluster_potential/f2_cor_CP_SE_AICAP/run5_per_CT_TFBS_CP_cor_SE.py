...
def scatter_plot(x,y,xlabel,ylabel,figname,title):
    ...
    if extra_index_num>0:
        label_indexes = label_indexes.append(y.loc[y.index.difference(current_index)].sort_values(ascending=False)[:extra_index_num].index)
    ...
    plt.text(.45,.01,'$R^2$ = {:.2f}'.format(r_value**2),fontsize=11,transform=plt.axes().transAxes)
