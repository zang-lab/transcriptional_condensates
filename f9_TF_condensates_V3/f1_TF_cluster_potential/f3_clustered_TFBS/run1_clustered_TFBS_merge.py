...
for percentile_thre in percentile_thres[:1]:
    ...
    # os.makedirs(outdir+os.sep+'bedfile',exist_ok=True)
    ...
    for tfbs_prename in tfbs_prenames[:1]:
        ...
        for index in df.index[:]:
            ...
            data_file = '../../../f8_TF_condensates_V2/f1_TF_cluster_potential/f1_bedtools_closest//data_{}/{}_sort_peaks.narrowPeak.tsv'.format(tfbs_prename,data_id)
            ...
            print(commandLine);os.system(commandLine)
            df.loc[index,'#TFBS_all'.format(percentile_thre)] = data_df.shape[0]
