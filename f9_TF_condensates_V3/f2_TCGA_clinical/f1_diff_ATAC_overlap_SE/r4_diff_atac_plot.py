...
def mark_pvalue(compr_pos,positions,box_vals):
    ...
    y, h, col = np.percentile(np.append(box_vals[compr_pos[0]],box_vals[compr_pos[1]]),99.96)*0.95 ,1.1, 'k'
...
for cancertype in name_match.index[:]:
    ...
    overlap_file = '{}/{}_ATAC_overlap_{}_SE.bed'.format(indir,cancertype,cancertype_SE_rename)
    nonoverlap_file = '{}/{}_ATAC_NOT_overlap_{}_SE.bed'.format(indir,cancertype,cancertype_SE_rename)
    ...
    plt.axes().set_xticklabels(['Overlapped w/ \n {} SE'.format(cancertype_SE_rename),
                                'Not overlapped w/ \n {} SE'.format(cancertype_SE_rename)],
