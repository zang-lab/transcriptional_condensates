...
for cancertype in name_match.index[:]:
    ...
    # == cancer data
    cancer_col = df_data.columns.intersection(filtered_id)
    cancer_data = df_data[cancer_col]
    out_df.loc[cancertype,'#samples'] = len(cancer_col)
    # == control data
    control_col = df_data.columns.difference(cancer_col)
    control_data = df_data[control_col]
    out_df.loc[cancertype,'#control'] = len(control_col)
...
out_df.to_csv(outdir+os.sep+'_data_diff_atac.csv')    
