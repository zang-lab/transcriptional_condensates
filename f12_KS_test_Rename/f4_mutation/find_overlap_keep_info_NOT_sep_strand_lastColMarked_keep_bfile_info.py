        ...
        if chrom not in regions:
            ...
            for island in islands[chrom]:              
                ...
                if s > e:
                    ...
                    col_poses = [float(regions[chrom][i][2].split('\t')[1]) for i in np.arange(e,s)]#  HERE IS THE ADDED COLUMN
                    col_info = np.sum(col_poses)#  HERE IS THE ADDED COLUMN
                    
                    #col_info = ','.join(regions[chrom][i][2].split()[0] for i in np.arange(e,s))#  HERE IS THE ADDED COLUMN
                    island[2] = island[2]+'\t'+str(col_info)+'\t'+str(len(col_poses))
                    if chrom not in overlapped:
                        overlapped[chrom] = []
                    overlapped[chrom].append(island)
                else:
...
def write_regions_bed3(regions1,regions2,outfile):
    ...
    with open(outfile,'w') as outf:
        outf.write('{}\t{}\t{}\t{}\t{}\n'.format('#chrom','start','end','IfOverlap','id\tmutationRate\tmutationCount'))
        for chrom in regions1:
            for region in regions1[chrom]:
                outf.write('{}\t{}\t{}\t1\t{}\n'.format(chrom,region[0],region[1],region[2]))
        for chrom in regions2:
