...
outdir = 'f2_clinical_per_gene_by_SE_RP'
...
for cancertype in ['BRCA','COAD'][:]:
    # ==== read clinical data
    # rp_df = pd.read_csv('data/ATAC_overlap_SE_per_patient_sig_RP_combined/BRCA_ATAC_overlap_MCF-7_SE_RP_combined.tsv',sep='\t',index_col=0)
    cancertype_SE = name_match.loc[cancertype].SE
    cancertype_SE_rename = name_match.loc[cancertype].SE_rename
    rp_file = 'data/ATAC_overlap_SE_per_patient_sig_RP_combined/{}_ATAC_overlap_{}_SE_RP_combined.tsv'.format(cancertype,cancertype_SE)
    ...
    clinical_df.to_csv('{}/{}_clinical_info.csv'.format(outdir,cancertype))
    ...
    out_df.to_csv(outdir+os.sep+'{}_SE_RP_add_clinical.csv'.format(cancertype))
