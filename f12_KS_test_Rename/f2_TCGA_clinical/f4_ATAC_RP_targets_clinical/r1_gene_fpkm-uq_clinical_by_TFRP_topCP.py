...
outdir = 'f1_clinical_by_TF_RP_targets'
...
for data_type in ['SE','CP'][1:]:
    for cancertype in ['BRCA','COAD'][:]:
        ...
        rank_df = pd.read_csv('{}/_CP_TFBS_all_vs_TFMS_{}.csv'.format(rank_dir,cancertype_SE),index_col=0)
        ...
        selected_tfs = rank_df.index[:tf_thre] ;print(selected_tfs)
        # if data_type == 'SE':
        ...
        for motif_name in selected_tfs[:]:
            ...
            if 1:
                ...
                rp_df = rp_df.loc[rp_df.index.intersection(clinical_df.index)]
        ...
        for count_thre in np.arange(3,4):
        ...
        #     # pvalues = df[df['treat time']<df['ctrl time']]['log rank p'].values
            ...
            xticklabels.append('CoTargets'.format(int(count_thre)))
