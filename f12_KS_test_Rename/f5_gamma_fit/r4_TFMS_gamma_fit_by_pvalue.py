...
outdir = 'f4_TFMS_gamma_alpha_by_pvalue'
...
for infile in infiles[:]:
    ...
    for p_thre in p_thres:
        try:
            df = pd.read_csv('{}/{}_p{}.tsv'.format(indir,outname,p_thre),sep='\t',header=None)
            ...
            out_df.loc[outname,'alpha p{}'.format(p_thre)] = alpha
            # out_df.loc[outname,'loc'] = loc
            out_df.loc[outname,'scale p{}'.format(p_thre)] = scale
            out_df.loc[outname,'#TFMS p{}'.format(p_thre)] = len(values)
        except:
            print('error',p_thre,outname) # in case of 0-line file
