...
def return_distance_distribution(afile,bfile,outdir,outname,flag):
    ...
    commandLine = 'bedtools closest -a {} -b {} -D ref -io -t first > {}\n'.format(bfile,afile,closest_file)
    os.system(commandLine);print(commandLine)  

    df = pd.read_csv(closest_file,sep='\t',header=None,low_memory=False)
    df = df[df[0].isin(hg38_chroms)]
    df.to_csv('{}/tmp/{}_{}.closest.bed.csv'.format(outdir,outname,flag),sep=',')
    col = df.columns[-1]
...
def compr_pairwise_distance(df,peak_file,treat_file,control_file,outdir,outname,labels,colors): 
    ...
    if int(outname.split('sample')[1]) ==0:
        ...
        sns.distplot(values_log_control,hist=False,kde=True,color=colors[1],label=labels[1],hist_kws={"lw": .0})
        sns.distplot(values_log_treat,hist=False,kde=True,color=colors[0],label=labels[0],hist_kws={"lw": .0} )
        plt.title(outname.split('_sample')[0])
        plt.legend(fontsize=10,borderaxespad=0.1,labelspacing=.1,
                    handletextpad=0.1,handlelength=1,
                    loc="upper left",
                    frameon=False)
        plt.ylabel('Density of sites')
        plt.xlabel('Distance to closest binding (log$_{10}$ bp)')
        s,p = stats.ttest_ind(values_log_control,values_log_treat)
        if s>0 and p<0.05:
            plt.text(.1,.5,'$p$={:.0e}'.format(p),transform=plt.axes().transAxes)
        else:
            plt.text(.1,.5,'N.S.'.format(p),transform=plt.axes().transAxes)
        # plt.xlim([0,9])
...
# ==== main
outdir = 'f2_gained_sampled_CBS_to_coBinding'
os.makedirs(outdir+os.sep+'tmp',exist_ok=True)

cancertypes = ['COAD','BRCA']


for bindingType in ['coBinding','Union']:
    df_out_summary = pd.DataFrame()
    for cancertype in cancertypes:
        # get a/b files for bedtools closest    
        peak_file = 'data/percentile_T_{}_{}.sort.bed'.format(cancertype,bindingType)
        treat_file = 'data/{}_gained.sort.bed'.format(cancertype)
        control_file = 'data/union_CBS_occupancy_GT3.sort.bed'
        # ==== outcome info
        labels = ['Gained CBS','Sampled CBS']   
        colors = ['tab:red','tab:grey'] 
        df_out = pd.DataFrame()
        for ii in np.arange(10): 
            outname_ii = '{}_{}_sample{}'.format(bindingType,cancertype,ii)   
        ...
        df_out.to_csv('{}/{}_{}.csv'.format(outdir,bindingType,cancertype))
        df_out_summary = pd.concat([df_out_summary,df_out.median()],axis=1)
    
    df_out_summary.columns = cancertypes
    df_out_summary.to_csv('{}/{}_summary.csv'.format(outdir,bindingType))
