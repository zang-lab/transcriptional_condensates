...
for profileType in profileTypes[:]:
    ...
    outdir = 'f9_per_CT_TFBS_CP_cor_zscore_CP_{}'.format(profileType)
    ...
    for cp_type in cp_types[1:]:
        ...
        for ct in tfbs_cp_s.columns[:]:
            ...
            z = tfbs_se_s[ct].loc[x.index]
            # x = tfbs_se_s[ct].loc[y.index]
            # x = np.log2(x)
            xlabel = 'Z-scored CP'
            ylabel = 'TFBS CP'
            zlabel = 'SE enrichment'
            # if cp_type == 'TFMS_CP':
            ...
            x_sort = np.sort(x)
            # plt.plot(x_sort,x_sort*slope+intercept,c = 'grey',ls='--',lw=.6)
            plt.text(.6,.06,r'$r = {:.2f}$'.format(r_value),
                     transform=plt.axes().transAxes)
            plt.axhline(y=0,color='grey',lw=.7,ls='--')
            plt.axvline(x=0,color='grey',lw=.7,ls='--')
            plt.xlabel('CP Z-score')
            # plt.ylabel(ylabel)
            # plt.ylim(0)
            # plt.ylim([0,130])
            # if not title=='GM12878':
