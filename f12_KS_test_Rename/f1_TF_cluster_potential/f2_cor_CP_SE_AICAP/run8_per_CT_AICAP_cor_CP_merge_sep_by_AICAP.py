...
for profileType in profileTypes[:]:
    ...
    outdir = 'f8_per_CT_cor_AICAP_cor_CP_{}'.format(profileType)
    ...
    for ct in tfbs_cp_s.columns[:]:
        ...
        label_i=1
        y_lower = ylim_max*.95
        label_indexes = x.sort_values(ascending=False)[:half_len].index
        label_indexes_sort = y[label_indexes].sort_values(ascending=False).index
        for label_index in label_indexes_sort:
            plt.scatter(x[label_index],y[label_index],
                        # c=colors2[label_i],
                        c=color,
                        s=25,label=label_index)
            ...
            dx= -.2 if label_i<=2 else -.2
            dy= .06 if label_i<=3 else -.06
            # dx,dy = .5,.1
            y_adjust = min(basey+dy,y_lower-.03)
            y_lower = y_adjust
            plt.axes().annotate(label_index, 
                         ...
                         # ha='left', va="center",
                         # size=110,
                         arrowprops=dict(color='w',lw=.0,arrowstyle="-"),
                         )
            label_i+=1
    ...
    #     ax.text(5, box_step[ii]+0.03,  p_val, fontsize=25,c='red',ha='center',va='center')
