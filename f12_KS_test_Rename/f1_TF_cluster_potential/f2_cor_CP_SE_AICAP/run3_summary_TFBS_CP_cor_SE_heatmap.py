...
fisher_P_key = ['data_median', 'data_fisher'][1]
profileTypes = ['with_motif_SE','with_motif_only']


for profileType in profileTypes[:1]:
    outdir = 'f3_TFBS_CP_heatmap_{}'.format(profileType)
    os.makedirs(outdir+os.sep+'_csv',exist_ok=True)
    for indir in indirs[:]:
        infile = '../f1_TFMS_TFBS_CP/{}/per_TF_per_Celltype_{}.csv'.format(indir,indir)
        df = pd.read_csv(infile,index_col = 0)
        correlation_data = {}
        correlation_p = {}
        # === data for CP and SE enrichment
        for or_key in ['CP_KStest','SE'][:]:
            # for fisher_P_key in ['data_median', 'data_fisher'][1:]:
            ...
            # print(p_col,base name)
            for ii in df.index:
                factor = ii.split('_')[1]
                celltype = ii.split('_')[0]
                if profileType == 'with_motif_SE':
                    if celltype not in SEs:
                        continue
            ...
            df_s_z = pd.DataFrame(index=df_s.index,columns=df_s.columns)
            for ii in df_s.index:
                if len(df_s.loc[ii].dropna())>1:
                    df_s_z.loc[ii] = scipy.stats.zscore(df_s.loc[ii],nan_policy='omit')
                else:
                    df_s_z.loc[ii] = np.abs(np.sign(df_s.loc[ii]))-1
            ...
            # save the data in the heatmap
            df_s.to_csv('{}/{}_S.csv'.format(outdir,basename))
            df_p.to_csv('{}/{}_P.csv'.format(outdir,basename))
        ...
        # plot the correlation of CP vs. SE
        cor_df = pd.DataFrame()
        x = correlation_data['CP_KStest'].to_numpy().flatten()
        x = x[~np.isnan(x)]
        xp = correlation_p['CP_KStest'].to_numpy().flatten()
        xp = xp[~np.isnan(xp)]
        
        y = correlation_data['SE'].to_numpy().flatten()
        y = y[~np.isnan(y)]
        yp = correlation_p['SE'].to_numpy().flatten()
        yp = yp[~np.isnan(yp)]
        z = [min(i) for i in zip(xp,yp)]
        z = -np.log10(z)
        cor_df['TFBS CP'] = x
        cor_df['SE'] = y
        cor_df['log min p'] = z
        cor_df = cor_df.sort_values(by='TFBS CP')
        cor_df.to_csv('{}/{}_{}_correlation_data.csv'.format(outdir,fisher_P_key,indir))
        
                
        # scatter for correlation
         plt.figure(figsize=(3,3))
        g = plt.scatter(x,y,c=z,cmap = plt.cm.GnBu,s=15,marker='o',edgecolors='k',lw=.5)
        slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)
        # s,p = stats.spearmanr(x,y)    
        x_sort = np.sort(x)
        plt.plot(x_sort,x_sort*slope+intercept,c = 'grey',ls='--',lw=1)
        ...
        cbar = plt.colorbar(g)
        cbar.ax.set_position([.85,.4,.4,.4]) 
        cbar.ax.tick_params(labelsize=10)
        cbar.set_ticks([10,100,200,300])
        
        cbar.set_label('-log10 Pvalue',labelpad=-45,fontsize=10)
        plt.xlabel('TFBS CP')
        plt.ylabel('Enrichment at SE \n (log2 Odds Ratio)')
        plt.title(' '.join(indir.split('_')[2:-2]))
        plt.savefig('{}/{}_{}_correlation.pdf'.format(outdir,fisher_P_key,indir),bbox_inches='tight',pad_inches=0.02,transparent=True)
        plt.show()
        plt.close()
