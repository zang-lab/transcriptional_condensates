...
def scatter_plot(df,xlabel,ylabel,figname,title):
    ...
    # label the TFs
    # label_i=0
    # top_y = df['TFBS CP'].sort_values(ascending=False)[:half_len].index
    ...
    label_i=0
    y_lower = .5
    y_max = abs(max(plt.axes().get_ylim(), key=abs))*.9
    y_step = y_max/5
    x_max = abs(max(plt.axes().get_xlim(), key=abs))
    # top_y = df['avg rank'].sort_values(ascending=True)[:half_len].index
    # top_y = df.loc[top_y][df['Z-scored CP'][top_y]>=0].index
    # top_y = df['TFBS CP'][top_y].sort_values(ascending=False).index
    # a = df['TFBS CP'].rank(ascending=False)
    # b = df['log2 Odds Ratio'].rank(ascending=False)
    # df['avg rank by SE'] = (a+b)/2
    # top_y = df['avg rank by SE'].sort_values(ascending=True)[:half_len].index
    top_y = df['TFBS CP'].sort_values(ascending=False)[:half_len].index
    # label_indexes = x.loc[top_y.intersection(x[x>0].index)].sort_values(ascending=False).index[:half_len]
    for label_index in top_y:
        plt.scatter(x[label_index],y[label_index],
                    # c=colors1[label_i],
                    c = color,
                    s=30,label=label_index)
        basex = x[label_index]
        basey = y[label_index]
        # # (dx,dy) = (.4,.1) if label_i<4 else (.4,-.1)
        # dx= -.5 if label_i<=3 else .5
        # dy= .1 if label_i<=3 else -.06
        dx = x_max/2.2
        # y_adjust = min(basey,y_max-y_step)
        y_adjust = y_max
        y_max = y_adjust-y_step
        plt.axes().annotate(label_index, 
                      xy=(basex,basey), 
                      xytext=(basex+dx,y_adjust),color='k',
                      # xytext=(x_max,y_adjust),color='k',
                      # ha='left', va="center",
                      # size=110,
                      arrowprops=dict(color='k',lw=.7,arrowstyle="-"),
                      )
        # label_i+=1
        label_i+=1
