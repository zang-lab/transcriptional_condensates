...
def scatter_plot(value_dic,xlabel,ylabel,key_pairs,mark=False):
    ...
    plt.text(.6,.1,'$R^2$ = {:.2f}'.format(r_value**2),fontsize=12,transform=plt.axes().transAxes)
    # ==== mark the outliers
...
xlabel,ylabel = 'Motif num','Motif length'
scatter_plot(value_dic,xlabel,ylabel,key_pairs)


key_pairs = ['KS_statistics','Motif_num']
xlabel,ylabel = 'TFMS CP','Motif num'
scatter_plot(value_dic,xlabel,ylabel,key_pairs)

key_pairs = ['KS_statistics','Motif_length']
xlabel,ylabel = 'TFMS CP','Motif length'
scatter_plot(value_dic,xlabel,ylabel,key_pairs)
