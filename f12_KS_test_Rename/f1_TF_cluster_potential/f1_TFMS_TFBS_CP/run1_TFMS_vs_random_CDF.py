...
def compr_sites_distribution(df_cp,infile,outdir): 
    ...
    # sns.distplot(sample_vals_log,kde=True,color='tab:grey',label='Random',hist=False,kde_kws={'cumulative':True} )
    # sns.distplot(values_log,kde=True,color='tab:green',label='TFMS',hist=False,kde_kws={'cumulative':True})
    sns.kdeplot(sample_vals_log, color='tab:grey',label='Random',cumulative=True)
    sns.kdeplot(values_log, color='tab:green',label='TFMS',cumulative=True)
    plt.title(outname)
...
for infile in infiles[:]:
    ...
    try:
        ...
        print('error') # in case of 0-line file

df_cp.index.name = 'TF'
df_cp = df_cp.sort_values(by='log10-dis ks_2samp-s',ascending=False)
