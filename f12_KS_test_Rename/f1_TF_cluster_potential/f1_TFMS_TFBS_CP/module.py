...
def compr_cluster_potential(df,treat_file,control_file,outdir,outname,labels,colors): 
    ...
    s,p = stats.ks_2samp(values_log_control,values_log_treat,alternative='less')
    df.loc[outname,'log10-dis ks_2samp-s less'] = s
    df.loc[outname,'log10-dis ks_2samp-p less'] = p 
    s,p = stats.ks_2samp(values_log_control,values_log_treat,alternative='greater')
    df.loc[outname,'log10-dis ks_2samp-s greater'] = s
    df.loc[outname,'log10-dis ks_2samp-p greater'] = p 
...
def enrichment_odds_ratio(df,treat_file,control_file,base_file,outname,labels):
    ...
    if os.path.isfile(base_file):
        ...
        df.loc[outname,'{} overlap SE'.format(labels[0])] = 0
        df.loc[outname,'{} overlap SE'.format(labels[1])] = 0
        # df.loc[outname,'{} total'.format(labels[0])] = treat_total
        # df.loc[outname,'{} total'.format(labels[1])] = control_total
        df.loc[outname,'fisher_exact_s'] = 0
        df.loc[outname,'fisher_exact_p'] = 0 
        return df
