...
def compr_sites_distribution(df_cp,infile,outdir): 
    ...
    # sns.distplot(sample_vals_log, kde=True,color='tab:grey',label='Random',hist_kws={"lw": .0},kde_kws={'cumulative': True} )
    # sns.distplot(values_log, kde=True,color='tab:purple',label='TFMS',hist_kws={"lw": .0},kde_kws={'cumulative': True})
    sns.kdeplot(sample_vals_log, color='tab:grey',label='Expected',cumulative=True,lw=3.5)
    sns.kdeplot(values_log, color='tab:purple',label='Observed',cumulative=True,lw=3.5)
    plt.title(outname)
    plt.legend(fontsize=13,borderaxespad=0.2,labelspacing=.2,
               handletextpad=0.2,handlelength=1,
               # loc="upper left",
               frameon=False)
    plt.ylabel('Cumulative Fraction',fontsize=16)
    plt.xlabel('Distance to \n nearest site (log$_{10}$ bp)',fontsize=16)
    plt.xlim([0,6])
...
# indf = pd.read_csv('CP_TFMS_vs_random/data_CP_TFMS_vs_random.csv'.format(outdir),index_col=0)
indf = pd.read_csv('../f2_cor_CP_SE_AICAP/data/TFMS_CP_SE_enrich.csv'.format(outdir),index_col=0)
indf = indf.sort_values(by='log10-dis ks_2samp-s signed',ascending=False)

df_cp = pd.DataFrame()
for prename in indf.index[:5]:
    infile = '..//f0_bedtools_closest/data_TFMS_jarspar/{}.tsv'.format(prename)
        # print(infile)
