...
for indir in indirs[:]:
    ...
    for tf in sorted(tfs):
        ...
        df_out2.loc[tf] = df.mean()
        for pcol in ['log10-distance CP pvalue',
                     'fisher_exact_p',
                     'log10-distance CP pvalue Fisher-data-combined',
                     'fisher_exact_p Fisher-data-combined']:
            freedom = 2*df.shape[0]
    ...
    df_out2 = pd.DataFrame(columns = df_out.columns)
    tfs = set([i.split('_')[1] for i in df_out.index])
    celltypes = set([i.split('_')[2] for i in df_out.index])
    for tf in sorted(tfs):
        for celltype in celltypes:
            row_name = '{}_{}'.format(tf,celltype)
            data_index = [i for i in df_out.index if re.search('_{}_{}'.format(tf,celltype),i)]
            df = df_out.loc[data_index]; # print(row_name,df.shape)
            if df.shape[0] ==0:
                continue
            df_out2.loc[row_name] = df.mean()
            for pcol in ['log10-distance CP pvalue',
                         'fisher_exact_p',
                         'log10-distance CP pvalue Fisher-data-combined',
                         'fisher_exact_p Fisher-data-combined']:
                ...
                sum_score = (-2*np.log10(df['{}'.format(pcol)])).sum()
                fisher_p = stats.chi2.sf(sum_score,freedom)
                df_out2.loc[row_name,'{} Fisher-TF-Celltype-combined'.format(pcol)] = fisher_p
        
    df_out2.to_csv('{}/per_TF_per_Celltype_{}.csv'.format(indir,indir))
