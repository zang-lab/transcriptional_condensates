...
# infiles = glob.glob('../f1_bedtools_closest/data_TFMS_jarspar//*CTCF*')
...
infile='../f1_bedtools_closest/data_TFMS_jarspar/CTCF_MA0139.1.tsv'
...
for ii in np.arange(1):
    ...
    print(ii,np.mean(dis_min),np.mean(dis_to_down))
    # continue
    
    # sample
    nums = df.shape[0]
    mean_val = 3298912062/df.shape[0]
    sample_vals = random.exponential(mean_val,nums)
    # sample_vals = scipy.stats.powerlaw(mean_val,nums)
    sample_vals = np.log10(sample_vals/2)
    
    
    # == plot the distribution of true/control data
    plt.figure(figsize=(3.6,3.6))
    sns.distplot(np.log10(dis_min+1),kde=True,color='tab:green',label='nearest-to-both',hist_kws={"lw": .0} )
    # sns.distplot( np.log10(dis_to_down+1), kde=True,color='tab:grey',label='nearest-to-down',hist_kws={"lw": .0})
    sns.distplot(sample_vals, kde=True,color='tab:purple',label='exponential',hist_kws={"lw": .0})
    # sample_vals = random.normal(loc=3,scale=1,size=1000)
    
    # sns.distplot(dis_min,kde=True,color='tab:grey',label='Random',hist_kws={"lw": .0} )
    # sns.distplot(values, kde=True,color='tab:green',label='TFMS',hist_kws={"lw": .0})
