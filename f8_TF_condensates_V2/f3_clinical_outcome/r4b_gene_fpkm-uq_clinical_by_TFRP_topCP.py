...
def stack_bar(box_vals,xticklabels,figname,cancertype,data_type):
    ...
    for position in positions:
        ...
        if p<0.05:
            ...
            plt.text(position,93*a/total, '*' , ha='center', va='bottom', color='r',fontsize=22)
    ...
    plt.title('{} {}'.format(cancertype,data_type))
    plt.axes().set_xticklabels(xticklabels,rotation=45, ha='right',fontsize=14,color='k')
    plt.ylabel('% genes w/ logrank P<0.05 ',fontsize=14)
...
outdir = 'f4b_gene_fpkm-uq_clinical_by_TFRP_topCP'
    ...
    for cancertype in ['BRCA','COAD'][:]:
        ...
        for motif_name in selected_tfs[:]:
            ...
            try:
                ...
                # df = clinical_df.loc[rp_df.index[:int(0.1*rp_df.shape[0])]].dropna()
                df = clinical_df.loc[rp_df[rp_df.RP>rp_thre].index].dropna()
