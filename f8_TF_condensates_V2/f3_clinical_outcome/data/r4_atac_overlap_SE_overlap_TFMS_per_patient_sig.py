...
outdir = 'ATAC_overlap_SE_overlap_TFMS_per_patient_sig'
    ...
    for motif_file in motif_infiles[:]:
        ...
        try:
            ...
            for patient in filtered_id[:]:
                ...
                # print(patient)
                bed_df['avg-score'] = sig_df[patient]
                bed_df.insert(3,'name',bed_df.index)
                bed_df = bed_df[['#seqnames', 'start', 'end', 'name', 'avg-score', 'annotation']]
                bed_df.to_csv('{}/{}_ATAC_overlap_{}_SE_overlap_{}_{}.bed'.format(outdir,cancertype,cancertype_SE,motif_name,patient),sep='\t',index=False,header=None)
        except:
            print(motif_name)
