...
def scatter_plot(x,y,xlabel,ylabel,figname,celltype,tfbs_per_data):

    # ==== scatter plot with density
    plt.figure(figsize=(2.6,2.6))
    x_linear,y_linear = [],[]
    for tf in y.index:
        per_data_index = [i for i in tfbs_per_data.index if re.search('_{}_{}'.format(tf,celltype),i)]
        y_new = tfbs_per_data.loc[per_data_index]['log10-distance CP t-stats']
        plt.scatter([x[tf]]*len(y_new),y_new,color='k',s=9)
        x_linear = np.append(x_linear,[x[tf]]*len(y_new))
    ...
    for label_index in label_indexes:
        per_data_index = [i for i in tfbs_per_data.index if re.search('_{}_{}'.format(label_index,celltype),i)]
        y_new = tfbs_per_data.loc[per_data_index]['log10-distance CP t-stats']
        plt.scatter([x[label_index]]*len(y_new),y_new,color=colors[label_i],
...
# == check the TF motifs
outdir = 'f2b_TFBS_CP_cor_TFMS_CP'
...
for tfbs_cp_type in tfbs_cp_types[:1]:
    ...
    per_data_cols = ['log10-distance CP t-stats','fisher_exact_s']
    ...
    # scatter_plot(x,y,xlabel,ylabel,figname,tfbs_cp_type,tfbs_per_data)
    ...
    for ct in tfbs_s.columns[:]:
        ...
        scatter_plot(x,y,xlabel,ylabel,figname,ct,tfbs_per_data)
