...
def scatter_plot(x,y,xlabel,ylabel,figname,title,per_data_cols):

    # ==== scatter plot with density
    plt.figure(figsize=(2.6,2.6))
    x_linear,y_linear = [],[]
    for ct in y.index:
        per_data_index = [i for i in tfbs_per_data.index if re.search('_{}_{}'.format(title,ct),i)]
        ...
        plt.scatter(x_new,y_new,color='k',s=8)
    ...
    for label_index in label_indexes:
        per_data_index = [i for i in tfbs_per_data.index if re.search('_{}_{}'.format(title,label_index,),i)]
        x_new = tfbs_per_data.loc[per_data_index]['fisher_exact_s']
        x_new = np.log2(x_new)
...
for tfbs_cp_type in tfbs_cp_types[:1]:
    ...
    for factor in tfbs_cp_s.index[:]:
        ...
        r_value, p_value = scatter_plot(x,y,xlabel,ylabel,figname,factor,per_data_cols)
        df.loc[factor,'R value'] = r_value.round(2)
