...
def scatter_plot(x,y,xlabel,ylabel,figname,title,tfbs_per_data):
    ...
    for tf in y.index:
        ...
        plt.scatter(x_new,y_new,c='k',s=9)
    ...
    for label_index in label_indexes:
        ...
        plt.scatter(x_new,y_new,color=colors[label_i],
    ...
    plt.text(.06,.88,'$R^2$ = {:.2f}'.format(r_value**2),fontsize=12,transform=plt.axes().transAxes)
...
half_len = 7
# norm = matplotlib.colors.Normalize(vmin=0,vmax=3*half_len)
norm = matplotlib.colors.Normalize(vmin=-1*half_len,vmax=3.5*half_len)
# pal = plt.cm.rainbow_r
pal = plt.cm.cubehelix_r
color_map = matplotlib.cm.ScalarMappable(norm=norm, cmap=pal)
colors = [color_map.to_rgba(i) for i in np.arange(2*half_len)]


outdir = 'f3b_sameCT_CP_cor_SE'
...
for tfbs_cp_type in tfbs_cp_types[:1]:
    ...
    for cp_type in cp_types[1:]:
        ...
        for ct in tfbs_cp_s.columns[:1]:
            ...
            if cp_type == 'TFMS_CP':
                y = tfms['log10 distance t-stats'].loc[y.index]
                ylabel = 'TFMS CP'
