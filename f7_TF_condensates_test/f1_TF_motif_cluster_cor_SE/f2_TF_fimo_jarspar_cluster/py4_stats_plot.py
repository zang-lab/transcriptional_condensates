...
# cutoff_OR2 = cutoff_OR2.fillna(0)
...
sns.distplot(cutoff_OR2[cutoff_OR2<=1000],kde=False)
# sns.distplot(cutoff_OR2,kde=False)
plt.ylabel('Number of TFs')
# plt.xlabel('Cutoff of Odds Ratio>2')
plt.xlabel('Candidate clustered TFBM cutoff')
# plt.xscale('log')
plt.savefig('{}/hist_OddsRatio.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.02,transparent=True)
plt.show()
plt.close()
print('w/ cutoff: ',cutoff_OR2.dropna().shape)
print('w/ cutoff <=300: ',cutoff_OR2[cutoff_OR2<=300].dropna().shape)
