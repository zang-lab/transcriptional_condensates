...
# ==== main  
indir = '../../f2_TCGA_ATAC_at_SE_cor_survival/f1_ATAC_overlap_SE_caseID'
...
for cancertype in name_match.index[:]:
    ...
    for region in sig_df.index[:]:
        ...
        # save info
        ix = df['group'] == 'treat'
        region_outdf.loc[region,'treat avg atac-score'] = df.loc[ix,'score'].values.mean().round(2)
        region_outdf.loc[region,'ctrl avg atac-score'] = df.loc[~ix,'score'].values.mean().round(2)
        ...
        region_outdf = region_outdf.sort_values(by='fdr')
