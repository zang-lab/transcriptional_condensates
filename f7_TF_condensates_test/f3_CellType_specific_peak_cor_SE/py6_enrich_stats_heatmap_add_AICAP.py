...
df_s = pd.read_csv('f5_cellType_specific_peaks_cor_SE_over_AllPeaks/fisher_s.csv',index_col=0)
df_p = pd.read_csv('f5_cellType_specific_peaks_cor_SE_over_AllPeaks/fisher_p.csv',index_col=0)
...
width_ratios = [4,.4,.1,.5]
gs = gridspec.GridSpec(3,4,width_ratios=width_ratios,wspace=.3,hspace=.4)
...
cbar.set_label('log2 (odds ratio)',labelpad=-55,fontsize=13)
# pvalue legend
...
plt.savefig('{}/log2_odds_ratio_heatmap_add_AICAP.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.02,transparent=True)
plt.show()
plt.close()
