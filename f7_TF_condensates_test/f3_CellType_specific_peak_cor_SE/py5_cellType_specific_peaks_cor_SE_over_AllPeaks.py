...
outdir = 'f5_cellType_specific_peaks_cor_SE_over_AllPeaks'
...
for factor in factors[:]:
    ...
    for celltype in factor_df.index[:]:
        celltype_specific_peak_file='f4_cellType_specific_peaks/{}_{}_specific.bed'.format(factor,celltype)
        celltype_all_peak_file = 'f3_union_peaks_across_cellTypes/{}_merge_intersect_{}.bed'.format(factor,celltype)
        # celltype_all_peak_file = 'f3_union_peaks_across_cellTypes/_{}_{}.bed'.format(factor,celltype)
        celltype_SE_file = '../f1_TF_motif_cluster_cor_SE/f4_cluster_enrichment_at_SE/data/SE_hg38/{}.bed'.format(celltype)
        if os.path.isfile(celltype_SE_file):            
