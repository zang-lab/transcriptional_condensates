...
project_dir = '/nv/vol190/zanglab/zw5j/since2019_projects/phase_separation_FEpiTR/f9_TF_condensates_V3'
project_dir = '/Volumes/zanglab/zw5j/since2019_projects/phase_separation_FEpiTR/f9_TF_condensates_V3'
diff_atac_dir = '{}/f2_TCGA_clinical/f1_diff_ATAC_overlap_SE/f9b_diff_ATAC_overlap_TFBS_clustered_data_figs'.format(project_dir)
atac_rp_dir = '{}/f2_TCGA_clinical/f4_RP_from_bigwig_cor_clinical/data_RP_merged/f4_avg_RP_per_sample_across_patients_figs'.format(project_dir)
...
name_match = pd.read_excel('{}/data/TCGA/TCGA-ATAC_SE_cancerType_match.xlsx'.format(project_dir),index_col=0)
name_match = name_match.dropna()

outdir = 'f1_ATAC_diff_RP_compr_C_NC_TFBS_heatmap'
...
rank_dir = '../../f1_TF_cluster_potential/f2_cor_CP_SE_AICAP/f9_per_CT_TFBS_CP_cor_zscore_CP/TFBS_CP/'
...
for treat_flag in treat_flags[:1]:
    for cancertype in cancertypes[:]:
        ...
        out_df.to_csv(outdir+os.sep+'data_{}_{}.csv'.format(cancertype,treat_flag))


        # == plot the heatmap
        plt.figure(figsize=(out_df.shape[0]/3.5,3))
        width_ratios = [1,.3,.4]
        height_ratios = [.35,.7,1,1,1]
        gs = gridspec.GridSpec(5,3,width_ratios = width_ratios,
        ...
        plt.savefig('{}/fig_{}_{}.pdf'.format(outdir,cancertype,treat_flag),bbox_inches='tight',pad_inches=0.02,transparent=True)
        plt.show()
        plt.close()
