...
for mutationType in ['Rate','Count']:
    for treat_flag in treat_flags[:1]:
        for ct in cts[:]:
            ...
            # cp_ranked_tf = tfbs_cp_s.loc[df.index,ct].sort_values(ascending=False).index
            # cp_ranked_tf = rank_df.loc[df.index].dropna().sort_values(ascending=False).index
