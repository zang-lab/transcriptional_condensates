...
def compr_sites_distribution(df_cp,infile,outdir): 
    ...
    sns.distplot(sample_vals_log, kde=True,color='tab:grey',label='Control',hist_kws={"lw": .0} )
    sns.distplot(values_log, kde=True,color='tab:purple',label='TFMS',hist_kws={"lw": .0})
    plt.title(outname)
...
infiles = [i for i in infiles if re.search('CENPB|EWSR1',i)]
infiles.sort()
