...
# ==== TFBS info
tfbp_cp_df = pd.read_csv('../f1_TFMS_TFBS_CP_V2_PeaksGT2k/CP_TFBS_all_vs_TFMS/per_data_CP_TFBS_all_vs_TFMS.csv',index_col=0)
...
tfbs_cp_type = 'CP_TFBS_all_vs_TFMS'

for test_type in test_types[2:]:
    # basename = '{}_{}_{}'.format(or_key,fisher_P_key,indir)
    basename = 'data_fisher_{}_CP_KStest'.format(tfbs_cp_type)
    ...
    for ct in tfbs_s.columns[:]:
        ...
        for factor in y.index[:]:
            kept_data = [i for i in tfbp_cp_df.index if re.search('{}_{}_'.format(ct,factor,),i)]
            if len(kept_data)<5 or factor == 'T':
                continue
            # print(ct,factor,len(kept_data))
            px = tfbp_cp_df.loc[kept_data,'#TFBS_all']
            px = np.log10(px)
            py = tfbp_cp_df.loc[kept_data,'log10-dis {}'.format(test_type)]
        ...
        plt.title(ct)
        if not color_i<2:
            plt.savefig('{}/per_ct_{}_{}.pdf'.format(outdir,ct,test_type),bbox_inches='tight',pad_inches=0.02,transparent=True)
            outdf.to_csv('{}/_per_ct_{}_{}.csv'.format(outdir,ct,test_type))
        plt.show()
        plt.close()
    ...
    # ==== TFBS CP for each CT
    for factor in tfbs_s.index[:]:
        y = tfbs_s.loc[factor].dropna()
        if len(y)<2:
            continue
        outdf = pd.DataFrame()
        plt.figure(figsize=(2.6,2.6))
        ...
        for ct in y.index[:]:
            ...
            px = np.log10(px)
            py = tfbp_cp_df.loc[kept_data,'log10-dis {}'.format(test_type)]
            plt.scatter(px,py,color=plt.cm.tab20(color_i),s=9,label=ct)
            slope, intercept, r_value, p_value, std_err = stats.linregress(px, py)      
            outdf.loc[factor,'r-value'] = r_value
            color_i+=1
            print(factor, py.min(),py.max())
        ...
        plt.title(factor)
        if not color_i<2:
            plt.savefig('{}/per_factor_{}_{}.pdf'.format(outdir,ct,test_type),bbox_inches='tight',pad_inches=0.02,transparent=True)
            outdf.to_csv('{}/_per_factor_{}_{}.csv'.format(outdir,ct,test_type))
        plt.show()
        plt.close()
