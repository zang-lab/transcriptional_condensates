...
def compr_cluster_potential(df,treat_file,control_file,outdir,outname,labels,colors): 
    ...
    df.loc[outname,'log10-dis T-test-s'] = round(s,2)
    df.loc[outname,'log10-dis T-test-p'] = '{:.2e}'.format(p) 
    s,p = stats.ranksums(values_log_control,values_log_treat)
    df.loc[outname,'log10-dis Wilcoxon-rank-sum-s'] = round(s,2)
    df.loc[outname,'log10-dis Wilcoxon-rank-sum-p'] = '{:.2e}'.format(p) 
    s,p = stats.ks_2samp(values_log_control,values_log_treat)
    df.loc[outname,'log10-dis ks_2samp-s'] = round(s,2)
    df.loc[outname,'log10-dis ks_2samp-p'] = '{:.2e}'.format(p) 
